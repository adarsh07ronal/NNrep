...
static void bkp_forward(bkp_network_t *n)
{
   ...
   for (o = 0;  o < n->NumOutputs;  o++) {
      ...
      n->LearningError = n->LearningError +
         ((n->OutputVals[o] - n->DesiredOutputVals[o]) * (n->OutputVals[o] - n->DesiredOutputVals[o])); // what is this
   }
   n->LearningError = n->LearningError / 2.0; //what is this
}
   ...
   for (o = 0;  o < n->NumOutputs;  o++) {
      ...
      for (h = 0;  h < n->NumHidden;  h++) {
         ...
         n->HiddenBetas[h] = n->HiddenBetas[h] +
            (n->HOWeights[h+(o*n->NumHidden)] * sigmoidDerivative(n->OutputVals[o]) * n->OutputBetas[o]); // what is this
